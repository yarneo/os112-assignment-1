...
main(int argc, char *argv[])
{
  ...
  25:	e8 d6 02 00 00       	call   300 <unlink>
...
stat(char *n, struct stat *st)
{
 ...
 20f:	e8 dc 00 00 00       	call   2f0 <open>
  if(fd < 0)
...
stat(char *n, struct stat *st)
{
 ...
 224:	e8 df 00 00 00       	call   308 <fstat>
  close(fd);
 ...
 22c:	89 c6                	mov    %eax,%esi
  close(fd);
 22e:	e8 a5 00 00 00       	call   2d8 <close>
  return r;
}
...
gets(char *buf, int max)
{
 ...
 274:	e8 4f 00 00 00       	call   2c8 <read>
    if(cc < 1)
...
putc(int fd, char c)
{
 ...
 3d6:	e8 f5 fe ff ff       	call   2d0 <write>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
...
putc(int fd, char c)
{
 ...
 4ad:	e8 1e fe ff ff       	call   2d0 <write>
 ...
 4ca:	e8 01 fe ff ff       	call   2d0 <write>
...
putc(int fd, char c)
{
 ...
 4fd:	e8 ce fd ff ff       	call   2d0 <write>
...
putc(int fd, char c)
{
 ...
 56d:	e8 5e fd ff ff       	call   2d0 <write>
      } else if(c == 's'){
        s = (char*)*ap;
        ap++;
        if(s == 0)
...
putc(int fd, char c)
{
 ...
 5a0:	e8 2b fd ff ff       	call   2d0 <write>
...
putc(int fd, char c)
{
 ...
 5f4:	e8 d7 fc ff ff       	call   2d0 <write>
...
morecore(uint nu)
{
 ...
 729:	e8 0a fc ff ff       	call   338 <sbrk>
  if(p == (char*) -1)
